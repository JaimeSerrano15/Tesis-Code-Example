...
def generate_labeled_structure_for_date(datacube, shapefile, date):
    datacube_for_date = datacube.sel(time=date)
    transform = calculate_transform(datacube_for_date)
    labeled_structure = create_labeled_structure(datacube_for_date, shapefile, transform)
    return labeled_structure


unique_dates = datacube['time'].values

labeled_structures = []
for date in unique_dates:
    labeled_structure = generate_labeled_structure_for_date(datacube, shapefile, date)
    labeled_structures.append(labeled_structure)

labeled_structures = np.stack(labeled_structures)
...
x = np.expand_dims(labeled_structures, axis=-1)
y = np.expand_dims(labeled_structures, axis=-1)

x_train, y_train, x_val, y_val, x_test, y_test = preprocess_data(x, y)

num_classes = len(np.unique(labeled_structures)) - 1
input_shape = (labeled_structures.shape[0], labeled_structures.shape[1], 1)
